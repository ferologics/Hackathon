        ...
        didSet {
            ...
            switch (state)
            {
                case .OffMode:
                    ...
                    for constraint in showSet { constraint.active = false }
                    
                    for constraint in hideSet { constraint.active = true }
                ...
                case .SearchMode:
                    ...
                    for constraint in showSet { constraint.active = true }
                    
                    for constraint in hideSet { constraint.active = false }
...
extension SearchViewController
{
    @objc func updateTableView(notification: NSNotification) { updateTableView() }
    
    func queryShizzle()
    {
        if self.searchCriteria.category == .Friends
        {
            HackathonHelper.getHackathonsFromParse( searchCriteria, onComplete: { (hackathons) -> Void in
